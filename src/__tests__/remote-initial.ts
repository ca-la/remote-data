...
test('getOrElseL', t => {
  ...
  t.is(initialRD.getOrElseL(() => 0), 0);
});

test('fold', t => {
  const { initialRD } = t.context;

  t.is(initialRD.fold(1, 2, () => 3, () => 4, () => 5), 1);
});

test('foldL', t => {
  const { initialRD } = t.context;

  t.is(initialRD.foldL(() => 1, () => 2, () => 3, () => 4, () => 5), 1);
});

test('altL', t => {
  const { initialRD, pendingRD, failureRD, successRD, refreshRD } = t.context;
  t.is(initialRD.altL(() => pendingRD), pendingRD);
  t.is(initialRD.altL(() => initialRD), initialRD);
  t.is(initialRD.altL(() => failureRD), failureRD);
  t.is(initialRD.altL(() => refreshRD), refreshRD);
  t.is(initialRD.altL(() => successRD), successRD);
});
...
test('recover', t => {
  ...
  t.deepEqual(initialRD.recover(f), initialRD);
});
